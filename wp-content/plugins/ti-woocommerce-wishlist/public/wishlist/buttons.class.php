...
class TInvWL_Public_Wishlist_Buttons {
	...
	private static function prepare(): array {
		...
		if ( $isColmCheckbox && $isColmActions ) {
			$buttons[] = [
				'name'      => 'product_apply',
				'title'     => sprintf( __( 'Áp dụng %s', 'ti-woocommerce-wishlist' ), "<span class='tinvwl-mobile'>" . __( '', 'ti-woocommerce-wishlist' ) . '</span>' ),
	...
	public static function apply_action_before(): string {
		$options = [
			'' => __( 'Chức năng', 'ti-woocommerce-wishlist' ),
		];
		...
		if ( $wishlist_current['is_owner'] ) {
			$options['remove_selected'] = __( 'Xóa', 'ti-woocommerce-wishlist' );
		}
