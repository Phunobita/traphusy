		...
		switch ( $action ) {
			case 'remove':
				...
				if ( $wlp->remove( $product_data ) ) {
					...
					$response['msg'][]  = sprintf(
						__( '%s đã bị xóa khỏi danh sách yêu thích.', 'ti-woocommerce-wishlist' ),
						$title
					);
				} else {
					$response['status'] = false;
					$response['msg'][]  = sprintf(
						__( '%s chưa bị xóa khỏi danh sách yêu thích.', 'ti-woocommerce-wishlist' ),
						$title
					);
				}
			...
			case 'add_to_cart_single':
				...
				if ( apply_filters( 'tinvwl_product_add_to_cart_need_redirect', false, $product_data_obj, $redirect_url, $product_data ) ) {
					...
					if ( $add && ! isset( $add['error_code'] ) ) {
						$response['status'] = true;
						$response['msg'][]  = sprintf(
							_n( '%s đã được thêm vào giỏ hàng của bạn.', '%s đã được thêm vào giỏ hàng của bạn.', 1, 'ti-woocommerce-wishlist' ),
							$title
						);
			...
			case 'remove_selected':
				...
				if ( ! empty( $titles ) ) {
					...
					$response['msg'][]  = sprintf(
						_n(
							'%s đã được xóa thành công khỏi danh sách yêu thích.',
							'%s đã được xóa thành công khỏi danh sách yêu thích.',
							count( $titles ),
			...
			case 'add_to_cart_selected':
				...
				if ( ! empty( $result ) ) {
					...
					/* translators: %s: product name */
					$response['msg'][] = sprintf( _n( '%s đã được thêm vào giỏ hàng của bạn.', '%s đã được thêm vào giỏ hàng của bạn.', $count, 'ti-woocommerce-wishlist' ), wc_format_list_of_items( $titles ) );
